...
@Recipe(pattern = {"xx", "xx"}, keys = @Recipe.Key(key = "x", value = "minecraft:polished_diorite"), id = DioriteBricks.ID, count = 4)
@BlockState(id = DioriteBricks.ID, variants = @BlockState.Variant(id = "", model = @BlockState.Model(DioriteBricks.LOCATION)))
@Model(id = DioriteBricks.ID, parent = "minecraft:block/cube_all", textures = @Model.Texture(key = "all", id = DioriteBricks.LOCATION))
@GenerateItem(DioriteBricks.ID)
@GenerateStairs(id = DioriteBricks.ID, texture = DioriteBricks.LOCATION)
@GenerateSlabs(id = DioriteBricks.ID, texture = DioriteBricks.LOCATION)
